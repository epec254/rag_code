...
# MAGIC
# MAGIC Can take 5 - 10 minutes.  Hide the results of this cell.
...
# MAGIC %md
# MAGIC
# MAGIC ## Available file parsers
# MAGIC
# MAGIC TODO: Improve these docs
# MAGIC
# MAGIC | Strategy                     | Params | Description                                  | Supported File Extensions    |
# MAGIC |------------------------------|--------|----------------------------------------------|------------------------------|
# MAGIC | `HTMLToMarkdownify()`          | None   | Converts HTML content to Markdown format.    | `.html`                      |
# MAGIC | `PassThroughNoParsing()`       | None   | Returns the input without any parsing.       | `.md`, `.txt`, `.html`                          |
# MAGIC | `PyMuPdfMarkdown()`            | None   | Converts PDF content to Markdown using PyMuPDF. | `.pdf`                     |
# MAGIC | `PyMuPdf()`                    | None   | Extracts text from PDF files using PyMuPDF.  | `.pdf`                       |
# MAGIC | `PyPdf()`                      | None   | Extracts text from PDF files using PyPDF.    | `.pdf`                       |
# MAGIC | `UnstructuredPDF()`            | [insert here]   | Handles PDF files that lack a clear structure. | `.pdf`                     |
# MAGIC | `PyPandocDocx()`               | None   | Converts DOCX files using Pandoc.            | `.docx`                      |
# MAGIC | `UnstructuredDocX()`           | None   | Manages DOCX files with unstructured content.| `.docx`                      |
# MAGIC | `UnstructuredPPTX()`           | None   | Manages PPTX files with unstructured content.| `.pptx`                      |
# MAGIC
# MAGIC
# MAGIC ## Available chunkers
# MAGIC
# MAGIC | Strategy                     | Params | Description                                  | Supported File Extensions    |
# MAGIC |------------------------------|--------|----------------------------------------------|------------------------------|
# MAGIC | `RecursiveTextSplitterByTokens()` | [insert here]   | Splits texts into chunks based on token count. | Any                      |
# MAGIC | `MarkdownHeaderSplitter()`     | [insert here] | Split texts based on Markdown headers | Any |
...
# TEMPORARY: for gte-large on dogfood, use endpoint `ep_05_08_release_rag_gte-large-en-v1_5_a10g`
# for bge, use `databricks-bge-large-en`


#TODO: improve the docs for this config
...
pipeline_configuration = {
    ...
    "embedding_model": {
        ...
        "endpoint": "ep_05_08_release_rag_gte-large-en-v1_5_a10g",
