...
# MAGIC %md
# MAGIC # RAG Document Prep Pipeline - From Delta Table
# MAGIC
# MAGIC This is an example notebook that provides a **starting point** to build a data pipeline that parses, chunks, and embeds text documents in a Delta Table into a Databricks Vector Search Index.  
# MAGIC
...
dbutils.widgets.text(
            ...
            label="#4 Delta Table w/ HTML content",
        )
source_delta_table = dbutils.widgets.get("source_delta_table")

# Columns within source Delta Table
dbutils.widgets.text(
            "html_column_name",
            defaultValue="",
            
            label="#5 Column Name w/ html_content",
        )
html_column_name = dbutils.widgets.get("html_column_name")

dbutils.widgets.text(
            "doc_uri_column_name",
            defaultValue="",
            
            label="#6 Column Name w/ doc_uri",
        )
doc_uri_column_name = dbutils.widgets.get("doc_uri_column_name")

# Validation
if (uc_catalog_name == "" or uc_catalog_name is None) or (uc_schema_name == "" or uc_schema_name is None):
    print("Please enter a UC Catalog & Schema to continue.")
else:
    print(f"Using `{uc_catalog_name}.{uc_schema_name}` as the UC Catalog / Schema.")

# Validation
if (source_delta_table == "" or source_delta_table is None) or (html_column_name == "" or html_column_name is None) or (doc_uri_column_name == "" or doc_uri_column_name is None):
    print("Please enter a Delta Table & the source column names to continue.")
else:
    print(f"Using `{uc_catalog_name}.{uc_schema_name}.{source_delta_table}` with `{html_column_name}` and `{doc_uri_column_name}` as the source columns.")
...
bronze_raw_files_table_name = (
    f"{uc_catalog_name}.{uc_schema_name}.{source_delta_table}__{tag}__bronze_raw"
)
silver_parsed_files_table_name = (
    f"{uc_catalog_name}.{uc_schema_name}.{source_delta_table}__{tag}__silver_parsed"
)
gold_chunks_table_name = (
    f"{uc_catalog_name}.{uc_schema_name}.{source_delta_table}__{tag}__gold_chunked"
)
gold_chunks_index_name = (
    f"{uc_catalog_name}.{uc_schema_name}.{source_delta_table}__{tag}__gold_chunked_index"
)
...
# MAGIC %md
# MAGIC ## Bronze: Load the documents from the source Delta Table

# COMMAND ----------

bronze_df = (
    spark.table(f'{uc_catalog_name}.{uc_schema_name}.{source_delta_table}')
)

# Rename the default column names to be more descriptive
bronze_df = (
    bronze_df.withColumnRenamed(doc_uri_column_name, DOC_URI_COL_NAME)
    .withColumnRenamed(html_column_name, CONTENT_COL_NAME)
)
...
def parse_file_wrapper(doc_uri, raw_doc_string_content, user_config):
    ...
    try:
        parsed_output = parser_class.parse_string(raw_doc_string_content)
...
if DEBUG:
  ...
  for sample in test_sample:
    test_output = parse_file_wrapper(test_sample[0][DOC_URI_COL_NAME], test_sample[0][CONTENT_COL_NAME], pipeline_configuration)
    print(test_output)
...
df_parsed = bronze_df.withColumn(
    ...
    parse_file_udf(F.col(DOC_URI_COL_NAME), F.col(CONTENT_COL_NAME)),
)
